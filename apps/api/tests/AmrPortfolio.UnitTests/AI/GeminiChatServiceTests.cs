...
public sealed class GeminiChatServiceTests
{
  ...
  // -----------------------------------------------------------------------
  // StreamResponseAsync — setup failure path
  // -----------------------------------------------------------------------

  [Fact]
  public async Task StreamResponseAsync_WhenRepoThrows_YieldsUnavailableError()
  {
    var repo = Substitute.For<IContentRepository>();
    repo.GetProfileAsync(Arg.Any<string>(), Arg.Any<CancellationToken>())
    ...
    var request = new ChatRequestDto("hello", [], "en", null);
    var events = new List<ChatEventDto>();

    await foreach (var evt in service.StreamResponseAsync(request))
      events.Add(evt);

    var single = Assert.Single(events);
    var error = Assert.IsType<ErrorEvent>(single);
    Assert.Equal(ChatErrorCodes.Unavailable, error.Code);
  }
}
